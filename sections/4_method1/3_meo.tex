...
\gls{sysa} assumes that \gls{id} class-relevant discriminative features are exclusively in either the foreground or the background view. It selects one of these views using a view selector before combining it with the original image for \gls{ood} detection. However, this assumption is rarely valid. \ref{fig:propwork-meo-motiv} gives two examples where \gls{sysa} fails to capture the entirety of discriminative features in either of the segmented view. In the first row, distinguishing components of the ``duck'' class are primarily in the foreground view but several important components are also in the background (e.g. the water and the reflections). In the lower section (second and third rows), the \gls{id} object discriminative features are all in the foreground views but important contextual features are in the background views. Domestic features in the background help contextualize the \gls{id} object of ``dog'' in the foreground against \gls{ood} object ``wolf'' which appears similar in the foreground but contains significantly different natural features in the background. Considering \emph{all views} in multi-view \gls{ood} detection can be beneficial.
...
\gls{sysb}, investigates this approach by generating features for all possible combinations of views. This set of view combinations (including the views themselves) is systematically evaluated for \gls{id} accuracy and \gls{ood} detection performance. Specifically, \gls{sysb} extracts view-specific features (``single-view features''), features fused from pairs of single-view features (``cross-view features''), and a cumulative feature resulting from the fusion of all single-view features (``comprehensive fused feature''). Each of these individual and fused features are learned end-to-end through an aggregate loss and used at test time for \gls{id} class prediction and feature-based \gls{ood} detection. All possible ensembles of individual and fused features are evaluated for contribution towards learning discriminative features and improving \gls{ood} detection.
...
\gls{sysb} resembles \gls{sysa} in that an \gls{mvcnn} is used for the FgBg multi-view model $f_{MV}$ backbone. Each feature extractor $\phi_v = f_{\theta,v}$ (parameterized by $\theta$) outputs a $d$-dimensional feature vector from input view $x_v$, $\phi_v: \mathcal{X} \mapsto \mathbb{R}^d$. The multi-view feature extractors $\phi_{MV}$ give $Z_{MV} = \{z_{org}, z_{fg}, z_{bg}\}$ with corresponding classification parameters $W_{MV}$ and $b_{MV}$ subsequently outputting $\hat{Y}_{MV} = \{\hat{y}_{org}, \hat{y}_{fg}, \hat{y}_{bg}\}$. The fusion component $\mathcal{U}$ in \gls{sysb} differs significantly from \gls{sysa}, instead constructing the single-view features, cross-view features, and comprehensive fused feature for ensembling.
...
The flow of information for \gls{sysb} is illustrated in  \ref{fig:propwork-meo-diagram}. The \gls{ist} segments the original image view (\textit{org}) into a foreground view (\textit{fg}) and a background view (\textit{bg}). The \gls{mvcnn} backbone generates features $Z_{MV}$ and logits $\hat{Y}_{MV}$. In \gls{sysb}, the fusion component $\mathcal{U}_{\text{\sysb}}$ is composed of three modules.

\subsubsection{Fusion Component}
The first module in \gls{sysb} $\mathcal{U}_{\text{\sysb}}$ is the Single-View Module which does not perform fusion, instead simply returning $Z_{MV}$ and $\hat{Y}_{MV}$ (i.e. a no-op).
...
The \gls{sysb} model is learned end-to-end using a view-aggregate loss $\mathcal{L}_{CE,agg}$. The loss aggregates the classification losses of all single-view, cross-view, and comprehensive fused feature-based predictions. At no point is any component or set of parameters in \gls{sysb} frozen while other parameters are trained.

A one-hot true label vector $y$ is used to train \gls{sysb} via $\mathcal{L}_{CE,agg}$ through each set of view features. Specifically, a view-specific CE loss $\mathcal{L}_{CE,sv}$ is defined for each $sv \in \{org, fg, bg\}$
\begin{equation}
\begin{aligned}
    \mathcal{L}_{CE,sv}(y, \hat{y}_{sv}) & = -\sum_{c \in \mathcal{Y}} y_{c} \log(\hat{y}_{sv,c}). \\
\end{aligned}
...
Following from \gls{sysa} (see Section \ref{sec:propwork-mod-propmeth}), \gls{sysb} leverages distance-based \gls{ood} detection methods to exploit the rich feature space learned from the use of foreground-background segmented views. \gls{sysb} is compatible with all distance-based (and otherwise post-hoc) \gls{ood} detection methods that operate on features and/or logits. However, this work only presents results for MD.

The implementation of MD for \gls{sysb} is very similar to that in \gls{sysa}. However, given that \gls{sysb} uses a combination of view-specific features, cross-view features, and a comprehensive fused feature, multiple empirical class means and covariance matrices must be defined. \gls{sysb} calculates empirical class means $\hat{\mu}_{C,v}$ and covariance matrices $\hat{\Sigma}_{v}$ for each view feature $v \in \{org, fg, bg, org+fg, org+bg, fg+bg, fused\}$. A test sample $x'$ is processed by \gls{sysb} into feature vector $z'_{v}$. Distances are computed between $z'_{v}$ and the nearest class-conditional Gaussian distribution for view $v$. The negative distance is the \gls{ood} score for view $v$
\begin{equation}
\begin{aligned}
...
All possible combinations of \gls{sysb} feature vectors $z_{sv}$, $z_{cv}$, $z_{fused}$ and prediction logits $\hat{y}_{sv}$, $\hat{y}_{cv}$, and $\hat{y}_{fused}$ are considered during test-time evaluation. An ensemble $\mathcal{E}$ is defined as the set of members composed of views, $\mathcal{E} \subseteq \{org, fg, bg, org+fg, org+bg, fg+bg, fused\}$. An ensemble $\mathcal{E}$ outputs prediction logits $\hat{y}_{e}$ and MD \gls{ood} scores $S_{MD,e}$ for each member $e \in \mathcal{E}$.
...
\gls{sysb} combines all ensemble $\mathcal{E}$'s member prediction through element-wise product of all softmax logits $\text{softmax}(\hat{y}_{e})$
\begin{equation}
\begin{aligned}
...
The experimental settings from \gls{sysa} are used for \gls{sysb}, with minor deviations (noted where appropriate). %Full results on all possible ensembles $\mathcal{E}$ are presented in the Appendix (see Appendix Section \ref{}{\color{red}TODO}).
...
CIFAR-10 and CIFAR-100 \cite{krizhevsky2009learning} are used as training and \gls{id} testing datasets. SVHN \cite{yuval2011reading} is added as a far-\gls{ood} dataset. ImageNet-30 \cite{russakovsky2015imagenet} is also added as a training and \gls{id} testing dataset. \gls{ood} datasets for ImageNet-30 follow previous experimental work \cite{li2023rethinking,yang2022openood} and include CIFAR-10, CIFAR-100, SVHN, LSUN, Textures, Places365, iNaturalist, and Tiny ImageNet. ImageNet-30 is not used as an \gls{ood} dataset for the CIFAR ID experimental settings because it has significantly higher resolution and image size, making the data domains incomparable.
...
\hline
\multicolumn{1}{|c|}{\multirow{2}{*}{}} & \gls{id} & \multicolumn{3}{c|}{\gls{ood}}    \\ \cline{2-5} 
...
\gls{sysa}-MD  & 95.37$_{\pm 0.29}$ & 95.54$_{\pm 5.39}$ & \multicolumn{1}{c|}{96.34$_{\pm 2.60}$}  & 95.94  \\ \cdashline{1-5}
\gls{sysb}-MD & & & & \multicolumn{1}{|c|}{} \\ %\cline{1-1} %\hline
...
\hline
\multicolumn{1}{|l|}{\multirow{2}{*}{}} &   \multicolumn{6}{c|}{\gls{ood}}   \\ \cline{2-7}
...
\gls{sysa}-MD  & / & 96.02$_{\pm 2.97}$ & 98.42$_{\pm 1.12}$ & 95.90$_{\pm 3.04}$ & 98.63$_{\pm 0.57}$ & \multicolumn{1}{|c|}{97.24} \\ \cdashline{1-7}
\gls{sysb}-MD & & & & & & \multicolumn{1}{|c|}{}  \\
...
\ref{tab:propwork-meo-results-cifar10-near} (near-\gls{ood} datasets) and \ref{tab:propwork-meo-results-cifar10-far} (far-\gls{ood} datasets) show results for the \gls{sysb} experiments on CIFAR-10. Only the top-3 \gls{sysb} ensembles are presented in this section. Previous state-of-the-art baselines are unable to outperform \gls{sysb} in \gls{ood} detection. 
In the near-\gls{ood} domain (\ref{tab:propwork-meo-results-cifar10-near}), the top \gls{sysb} ensembles outperform all baselines. In particular, the $f+O+F+OB$ ensemble improves the average AUC by 4.04\%, AUC of CIFAR-100 by 7.08\%, and AUC of Tiny ImageNet by 1.00\%. Across the top-3 ensembles, performance is consistent. Variance of AUC is only 0.02\% for CIFAR-100, 0.02\% for Tiny ImageNet, and 0.005\% on average. \gls{sysb} ensembles also outperform \gls{sysa}, validating the idea of utilizing feature information in \emph{all} views for \gls{ood} detection instead of selectively eliminating views. The $f+O+F+OB$ \gls{sysb} ensemble improves the average CIFAR-10 near-\gls{ood} AUC by 3.00\%, from 95.94\% to 98.94\%, a significant margin.
In the far-\gls{ood} domain (\ref{tab:propwork-meo-results-cifar10-far}), \gls{sysb} ensembles again improve AUC both on average (4.24\%) and across individual datasets (except SVHN), where the $f+O+F+OF+OB$ ensemble underperforms by only 1.56\% (and still achieves a high AUC of 98.34\%). On the challenging iNaturalist far-\gls{ood} dataset, \gls{sysb} ensembles perform extremely well, each improving AUC by 5.81-5.82\% over the nearest state-of-the-art baseline. Variance across the top-3 \gls{sysb} ensembles is again low, only 0.01\% for average \gls{ood} detection performance. Similar to the near-\gls{ood} CIFAR-10 datasets, \gls{sysb} ensembles improve on \gls{sysa}, improving average \gls{ood} detection by 2.21\% and demonstrating a particularly large increase in Places365 of 3.76\%.
\gls{id} classification is also unaffected by the multi-view paradigm. All 3 top \gls{sysb} ensembles improve \gls{id} classification accuracy (maximum improvement of 0.68\%), demonstrating that \gls{sysb} is able to learn \gls{id} class-relevant discriminative features effective for \gls{ood} detection while maintaining \gls{id} classification functionality.
...
\gls{sysa}-MD  & 78.72$_{\pm 0.34}$ & 81.66$_{\pm 6.28}$ & 91.19$_{\pm 4.61}$ & \multicolumn{1}{|c|}{87.12}   \\  \cdashline{1-5}
\gls{sysb}-MD & & & & \multicolumn{1}{|c|}{} \\ %\cline{1-1} %\hline
...
\gls{sysa}-MD & / & 96.64$_{\pm 2.49}$ & 95.20$_{\pm 1.64}$ & 97.40$_{\pm 1.27}$ & 98.53$_{\pm 0.84}$ & \multicolumn{1}{|c|}{96.44} \\  \cdashline{1-7}
\gls{sysb}-MD & & & & & & \multicolumn{1}{|c|}{}  \\
...
For the CIFAR-100 experiments (see \ref{tab:propwork-meo-results-cifar100-near} and \ref{tab:propwork-meo-results-cifar100-far}), \gls{sysb} similarly outperforms baselines and improves on \gls{sysa}. For near-\gls{ood} datasets (\ref{tab:propwork-meo-results-cifar100-near}), the top-3 ensembles significantly improve \gls{ood} detection performance. In particular, the $f+O+F+OF+OB$ ensemble improves CIFAR-10 AUC by 2.93\%, Tiny ImageNet AUC by 9.51\%, and average AUC by 9.82\% over the best baselines. With respect to \gls{sysa}, \gls{sysb} ensembles repeatedly demonstrate the value of including all views in learning and \gls{ood} detection. CIFAR-10 AUC increases by 1.98\% from \gls{sysa} to \gls{sysb} $f+O+F+OF+OB$, Tiny ImageNet AUC increases by 6.52\%, and average AUC increases by 3.54\%. Variance across ensembles is 0.11\% for CIFAR-10, 1.81\% for Tiny ImageNet, and 0.61\% for average near-\gls{ood} AUC. These values are larger than for CIFAR-10 but remain low. It is again observed that using multi-view methods for \gls{ood} detection, especially considering all views as in \gls{sysb}, significantly improves near-\gls{ood} detection in challenging domains such as CIFAR-100. The increase from 80.85\% with GEN to \textbf{90.67\%} with \gls{sysb} $f+O+F+OF+OB$ is significant.
For far-\gls{ood} datasets (\ref{tab:propwork-meo-results-cifar100-far}), \gls{sysb} ensembles continue to outperform baselines and \gls{sysa}. Across the far-\gls{ood} datasets, the average AUC is increased by 16.40\% with the $f+O+F+OF+OB$ ensemble. All of the top-3 \gls{sysb} ensembles outperform the baselines in each far-\gls{ood} dataset except SVHN. In iNaturalist, AUC increases by 15.84\% over the nearest baseline LogitNorm. The average AUC for \gls{sysb} outperforms \gls{sysa} by 1.40\%. The variance across \gls{sysb} ensembles remains relatively low, with an average AUC variance of 0.21\%. \gls{id} classification is again observed to be unaffected by inclusion of multiple views. In fact, \gls{sysb} $f+F+B+OB+FB$ improves \gls{id} classification accuracy by 0.97\% over the baselines and is comparable to \gls{sysa} (a difference of only 0.09\%).
...
\hline
\multicolumn{1}{|l|}{\multirow{2}{*}{}} & \gls{id} &  \multicolumn{5}{c|}{\gls{ood}} \\ \cline{2-7} 
...
\hline
\multicolumn{1}{|l|}{\multirow{2}{*}{}} &   \multicolumn{5}{c|}{\gls{ood}} \\ \cline{2-6} 
...
\gls{sysb} is additionally evaluated on the ImageNet-30 \gls{id} dataset which contains higher resolution images and a more challenging distribution of object classes. Compared to the other object datasets, e.g. CIFAR, iNaturalist, and Tiny ImageNet datasets (which are treated as near-\gls{ood} datasets), ImageNet-30 classes are more semantically rich and \emph{similar to near-\gls{ood} classes}. This introduces significant challenges for models which must learn discriminative features to detect \gls{ood} samples. 
Near-\gls{ood} datasets results are shown in \ref{tab:propwork-meo-results-inet30-near}. \gls{sysb} ensembles attain an average AUC of 97.84\%, an improvement of 5.15\% over the nearest baseline. Baselines slightly outperform \gls{sysb} on CIFAR-100 (1.12\%) and CIFAR-10 (0.12\%) but are generally weak on iNaturalist and Tiny ImageNet, which \gls{sysb} $f+O+F+OB$ improves upon by 14.74\% and 7.08\% respectively. Variance remains low across the top-3 \gls{sysb} ensembles. Average AUC variance for \gls{sysb} is 0.29\%.
The results for far-\gls{ood} datasets, presented in \ref{tab:propwork-meo-results-inet30-far}, show that all top-3 \gls{sysb} ensembles outperform previous baselines in average \gls{ood} detection, with the best ensemble $f+O+F+OB$ improving by 3.04\%. Performance increases are consistent across each individual \gls{ood} dataset except for SVHN, in which \gls{sysb} is only outperformed by 1.87\% (by KNN). Variance across \gls{sysb} ensembles for average AUC is again low at 0.15\%.
\gls{sysb} ensembles improve \gls{id} classification accuracy (see \ref{tab:propwork-meo-results-inet30-far}). Specifically, the $f+O+F+OF+OB$ ensemble increases accuracy by 6.52\%, with other ensembles performing similarly. The ImageNet-30 experiments test the ability of \gls{sysb} to handle images with higher resolution. The results demonstrate that \gls{sysb} can handle the rich feature information and more difficult near-OOD datasets \cite{koner2021oodformer,li2023rethinking}. 

\subsubsection{View Breadth}
Of interest is the observation that all 3 top \gls{sysb} ensembles utilize at least one view-specific feature, at least one cross-view feature, and the comprehensive fused feature. Specifically, there is a clear presence of $f$, $O$, $F$, and $OB$ ensemble members in the top-performing \gls{sysb} ensembles. This implies that \gls{sysb} relies on the rich information captured in single-view, cross-view, \textit{and} comprehensive fused features for improving \gls{ood} detection performance. This property of including single-view, cross-view, and comprehensive fused features, denoted as ``view breadth'', enables \gls{sysb} to achieve state-of-the-art performance.

The results across experimental settings, i.e. with CIFAR-10 \gls{id}, CIFAR-100 \gls{id}, and ImageNet-30 \gls{id}, reinforce the importance of view breadth for achieving state-of-the-art performance. All top-performing \gls{sysb} ensembles in \ref{tab:propwork-meo-results-cifar10-near}, \ref{tab:propwork-meo-results-cifar10-far}, \ref{tab:propwork-meo-results-cifar100-near}, \ref{tab:propwork-meo-results-cifar100-far}, \ref{tab:propwork-meo-results-inet30-near}, and \ref{tab:propwork-meo-results-inet30-far} exhibit view breadth.
This observation further verifies the hypothesis that eliminating view features (as in \gls{sysa}) is suboptimal compared to considering all available views.
...
\hline
\multicolumn{1}{|l|}{\multirow{2}{*}{}} &   \gls{id}  & \multicolumn{8}{c|}{\gls{ood}}   \\ \cline{2-10}
...
To better understand the performance of \gls{sysb} ensembles, an analysis is performed on the impact of the foreground single-view features (i.e. $F$), the background single-view features (i.e. $B$), and the original view features (i.e. $O$) on \gls{id} classification and \gls{ood} detection performances, both individually and in ensembles. \ref{tab:propwork-meo-results-single-view_compare} shows the results of all possible ensembles consisting of \textit{single-view features} for the CIFAR-100 \gls{id} setting. \ref{tab:propwork-meo-results-single-view_compare} gives the following observations: % We do not analyze the original single-view features (i.e. $O$) here because it does not exhibit the unique advantageous properties of either the $F$ and $B$ views. The $O$ view is beneficial in all regards as it contains all semantic information in the image. For this reason, we include it in this analysis as a reference for showing improvements from $F$ and $B$. 
\begin{enumerate}
    \item Without ensembling, $O$ gives the best, $F$ gives the second best, and $B$ gives the poorest \gls{id} classification performance. This is expected. However, $B$ gives the best \gls{ood} detection results at the expense of \textbf{significant instability}. This was observed in \gls{sysa}, and is further elaborated on below. 
    \item Among the ensembles $\{O+F, O+B, F+B\}$, $O+F$ has the best \gls{id} classification performance, with $O+B$ slightly less and $F+B$ the worst. However, the $O+B$ ensemble has the best \gls{ood} performance. These observations again show that $F$ is more useful for \gls{id} classification and $B$ is more useful for \gls{ood} detection. However, the benefits of $F$ and $B$ are \emph{contextual}. $F+B$ performs significantly worse in \gls{id} classification and slightly worse than $O+B$ in \gls{ood} detection. This indicates that $B$ improves \gls{ood} detection \emph{in combination with $O$}. 
    \item The ensemble $O+F+B$ has a similar \gls{ood} detection performance to $O+B$, which indicates again the importance of $B$ for \gls{ood} detection. However, it also verifies that intelligent usage of all foreground-background views (i.e. via principled fusion methods like \gls{sysa} and \gls{sysb}) improves \gls{ood} detection. Adding $F$ to $O$ and $B$ further improves \gls{ood} detection performance instead of degrading it. It also improves \gls{id} classification performance. Overall, $O+F+B$ is the best considering both \gls{id} classification and \gls{ood} detection.
    \item The $F$ and $B$ views are complementary.
\end{enumerate}

In summary, it can be concluded that $F$ is more useful for \gls{id} classification (which is expected) and $B$ is more useful for \gls{ood} detection, which is interesting and to some extent unexpected, although the $B$ view introduces significant instability. A possible explanation of the $B$ view performance effects builds on the analysis of \gls{sysa} (see Section \ref{sec:propwork-mod-propmeth}). During training, a standard single-view model focuses on learning features that discriminatively separate the \gls{id} classes, which means it attends more to the main object. For most object datasets and segmentation models (i.e. those considered in these studies), the discriminative features are predominantly in the foreground of the image. The background is sparse in discriminative features and instead predominantly contains \emph{contextual} and \emph{spurious} features. Note that these contextual features contain the information inherent to the foreground-background segmentation itself. While the foreground contains the \gls{id} class object, the background instead contains the \emph{shape} of the \gls{id} class object (via the ``silhouette'' left by segmentation) in addition to the other non-\gls{id} class object information in the image.

By splitting learning of image features into foreground- and background-view models, implemented as the feature extractor backbones $\phi_{fg}, \phi_{bg}$ and view-specific losses $\mathcal{L}_{CE,sv}$ in \gls{sysb} and $\mathcal{L}_{CE,v}$ \gls{sysa}, the foreground and background features are learned separately.
For the background, the background-view model learns the background feature distribution for the \gls{id} classes which simultaneously 1) identifies the \gls{id} contextual features (and occasionally discriminative features), 2) represents the \gls{id} space, and 3) rejects spurious features. The sparsity of \gls{id} features in this space gives rise to high \gls{ood} detection performance and uniform prediction distributions \emph{if the background-view model converges}. The background-view model only recognizes background features that align with the few discriminative and contextual features it has seen in the \gls{id} dataset, rejecting \emph{all other features} as \gls{ood}. However, the sparsity of these discriminative and contextual features in the \gls{id} training data distribution destabilizes learning. This causes the background-view model to either not converge or converge to non-general solutions. At test time, outputs from the background-view model for \gls{id} and \gls{ood} samples can vary wildly, causing high variance in performance. This issue of feature sparsity is exacerbated by the low-resolution images from the selected datasets. Manual verification revealed that segmentation resulted in many samples with completely empty background views. %Despite this, the qualitative study in Section {\color{red}TODO (\sysa)} on a high-resolution sample verifies the foreground-background performance benefits. %The collection of these behaviors across segmented views becomes very useful in OOD detection.

%The overall trend of foreground-background performance was validated using an empirical analysis. Table \ref{fig:propwork-meo-results-fgbg_contrib} shows a plot of MD distances for $O$, $F$, and $B$ for the CIFAR-100 \gls{id} testing set and Tiny ImageNet near-\gls{ood} dataset. There is a strong correlation between MD and dataset, especially for $B$. Distances remain low for both the $O$ and $F$ features between the \gls{id} and \gls{ood} datasets, indicating overconfidence in learned \gls{id} features. However, the sensitivity of the $B$ space to contextual and spurious features significantly increases the distance of \gls{ood} features with respect to the \gls{id} dataset, verifying that, when the background-view model converges, $B$ is beneficial for \gls{ood} detection and improves performance on near-\gls{ood} datasets.
...
Experiments with \gls{sysb} substantiate the claims that selectively including only one of the available foreground or background views, and thus eliminating the feature information in the excluded view, \emph{limits \gls{ood} detection performance improvements}. The 3 top-performing \gls{sysb} ensembles all included some amount of view-specific features, cross-view features, and the comprehensive fused feature. This is evidence for the benefits of including \emph{all} views in \gls{ood} detection. \gls{sysb} further improves on \gls{sysa} by 1) learning view features from additional feedback (i.e. cross-view features), and 2) removing the view selector, improving learning consistency and \gls{ood} detection stability.

\gls{sysb} considers all views during learning and \gls{ood} detection through computation of all individual views, all possible cross-view combinations, and a comprehensive fused feature. Furthermore, \gls{sysb} constructs and evaluates all possible ensembles over these views, resulting in \textbf{over 100 computations per forward pass}. This brute-force approach was shown to be more effective, but continues to exhibit significant shortcomings:
\begin{itemize}
    \item Inefficient Processing - The computational complexity of \gls{sysb} \gls{ood} detection is significant.
    ...
    \item Limited Experimental Evaluation - \gls{sysb} continues to only consider CIFAR as the \gls{id} dataset, with the inclusion of a small-scale ImageNet subset. There are many other large-scale dataset with higher-resolution, semantically complex images that have been demonstrated to be better suited for the proposed foreground-background segmented view \gls{ood} detection approach. As low-resolution segmentation is outside the scope of this dissertation, these large-scale experimental settings are more appropriate for evaluation.
\end{itemize}
Scaling the multi-view FgBg idea up to realistic experimental settings is necessary for the development of improved FgBg segmentation-based \gls{ood} detection solutions for open-world domains and applications.
