...
This section focuses primarily on the architectural improvements towards improved multi-view OOD detection. The data issues described in previous sections (i.e. for \gls{sysa} and \gls{sysb}) are not addressed here but in the next chapter (see Chapter \ref{diss_casod}). Instead, this section is an incremental improvement on \gls{sysa} that introduces and demonstrates the functionality and benefits of Attention in the proposed FgBg multi-view OOD detection domain.
...
The information flow for \gls{sysc} is presented in \ref{fig:propwork-scod-diagram} and follows that of \gls{sysa} closely, with the IST segmenting the original image view (\textit{org}) into foreground (\textit{fg}) and background (\textit{bg}) views, the MVCNN backbone generating features $\tilde{Z}_{MV}$, feature vectors $Z_{MV}$, and logits $\hat{Y}_{MV}$ from the views, and the fusion component $\mathcal{U}_{\text{\sysc}}$ combining view information into fused feature $\tilde{z}_{\mathcal{U}}$. Notably, $\mathcal{U}_{\text{\sysc}}$ differs from \gls{sysa} in the fusion mechanisms after view selection. Overlapping details are not mentioned in this section and instead should be assumed to mimic those in Section \ref{sec:propwork-mod-propmeth}.
...
The view selection operation in \gls{sysc} is identical to that in \gls{sysa}. It takes as input the foreground and background view-specific feature vectors $z_{fg}, z_{bg}$ and produces as output the view selection weight $\mu$. See Section \ref{sec:propwork-mod-propmeth} for details.
...
\end{aligned}
\end{equation}
In \gls{sysc}, $T_{v1} = T_{v2} = T$ and $d_k = d_v = d$.
...
Similar to \gls{sysa} and \gls{sysb}, the fused feature vector and view-specific feature vectors,
\begin{equation}
\begin{aligned}
...
The design of \gls{sysc} mimics that of \gls{sysa} and a similar, alternating optimization paradigm is used. Training objectives are targeted towards the feature extractors and the fusion component. The fusion component loss $\mathcal{L}_{VS}$ is similar to \gls{sysa} and targets learning of the view selector. However, the feature extraction objective improves on \gls{sysa} by introducing an alignment loss $\mathcal{L}_{con}$ in addition to the classification loss $\mathcal{L}_{CE}$.
...
The view selection loss $\mathcal{L}_{VS}$ in \gls{sysc} is identical to that in \gls{sysa}
\begin{equation}
\begin{aligned}
    E_{fg} = \text{RMSE}(\hat{y}_{fg}, y) \\
    E_{bg} = \text{RMSE}(\hat{y}_{bg}, y) \\
...
\gls{sysc} follows \gls{sysa} in that only $z_{\mathcal{U}}$ and $\hat{y}_{\mathcal{U}}$ are used for OOD detection. See Section \ref{sec:propwork-mod-propmeth} for details.

\subsection{Experiments}
\label{sec:propwork-scod-exp}

The experimental setup for \gls{sysc} uses the same datasets, image segmentation model, feature extractor architecture, baselines, and evaluation metrics as those in \gls{sysa}. The training details differ due to the introduction of a contrastive loss term to the optimization objective. Training settings are:
\begin{itemize}
    \item Batch Size = 64
...
\gls{sysc}-MD  & 94.37$_{\pm 0.33}$ &  94.19$_{\pm 1.80}$ & 96.72$_{\pm 0.81}$ & 95.48$_{\pm 1.06}$ & 96.26$_{\pm 1.28}$ & 98.17$_{\pm 0.95}$ & 95.76$_{\pm 0.76}$  & 96.66 & 94.98 &  96.10     \\ 
\gls{sysc}-KNN  & 94.37$_{\pm 0.33}$ & 92.00$_{\pm 1.55}$ & 95.00$_{\pm 0.41}$ & 94.61$_{\pm 1.18}$ & 93.46$_{\pm 0.67}$ & 97.12$_{\pm 1.39}$ & 94.04$_{\pm 1.36}$ & 95.05 & 93.02 &  94.37   \\ \hline
\end{tabular}
}
...
\gls{sysa}-MD & 78.72$_{\pm 0.34}$ & 81.66$_{\pm 6.28}$ & 96.64$_{\pm 2.49}$ & \textbf{95.20}$_{\pm 1.64}$ & 97.40$_{\pm 1.27}$ & 98.53$_{\pm 0.84}$ & \multicolumn{1}{c|}{91.19$_{\pm 4.61}$} & 96.44 & 87.12  &  93.33  \\ 
...
\gls{sysc}-MD & 72.09$_{\pm 1.02}$ &  \textbf{89.97}$_{\pm 2.18}$ & \textbf{98.16}$_{\pm 0.46}$ & 95.17$_{\pm 1.54}$ & \textbf{98.84}$_{\pm 0.64}$ & \textbf{98.92}$_{\pm 0.58}$ & \textbf{96.48}$_{\pm 0.81}$ & \textbf{97.77} & \textbf{93.22} &  \textbf{96.26} \\ 
\gls{sysc}-KNN & 72.09$_{\pm 1.02}$ & 88.36$_{\pm 6.15}$ & 96.16$_{\pm 2.39}$ & 95.08$_{\pm 2.31}$ & 96.97$_{\pm 1.90}$ & 98.23$_{\pm 0.75}$ & 95.85$_{\pm 2.22}$ & 96.61 & 92.11 &  95.11 \\ \hline
\end{tabular}
}
...
The addition of cross-attention to FgBg multi-view OOD detection provides interesting benefits. These benefits are illustrated in the changes across CIFAR-10 and CIFAR-100 datasets experimental results. Starting with CIFAR-10 (results of which are in \ref{tab:propwork-scod-results-cifar10}), \gls{sysc} outperforms all baselines on average and in many individual datasets. Overall, \gls{sysc}-MD improves AUC by 2.59\%. Across near-OOD datasets, \gls{sysc} improves AUC by 0.08\%, and across far-OOD datasets, \gls{sysc} improves AUC by 2.05\%. However, \gls{sysc} slightly underperforms compared to \gls{sysa}, 0.58\% less across far-OOD datasets, 0.96\% less across near-OOD datasets, and 0.71\% less on average. For the simpler CIFAR-10 domain (with fewer ID classes and more distinct semantic differences across classes), the additional discriminative feature learning capabilities of Attention-based mechanisms are less beneficial. Simply max-pooling the features is sufficient in identifying view-correlated ID class-relevant features. Cross-attention, while certainly still beneficial (\gls{sysc} still outperforms baselines), learns and emphasizes greater nuances in the feature signals of individual views which is unnecessary for the simple discriminative features of the CIFAR-10 dataset.

The advantage of \gls{sysc} becomes clear when considering the more-challenging CIFAR-100 dataset, results for which are in \ref{tab:propwork-scod-results-cifar100}. \gls{sysc} improves OOD detection performance across all OOD datasets except Textures (where performance differs from \gls{sysa} by only 0.03\%). For far-OOD datasets, \gls{sysc} improves AUC by 16.57\% over baselines and 1.33\% over \gls{sysa}. For near-OOD datasets, \gls{sysc} improves AUC by 10.28\% over baselines and 6.10\% over \gls{sysa}. On average, \gls{sysc} improves AUC by 14.48\% over baselines and 2.93\% over \gls{sysa}. Cross-attention greatly improves OOD detection in the CIFAR-100 ID domain where greater semantic overlap exists across classes, requiring precise discriminative feature extraction and learning. Leveraging cross-attention on the additional feature correlation in foreground and background views enables \gls{sysc} to achieve state-of-the-art OOD detection performance, and further outperform the naive max-pooling fusion in \gls{sysa}. This is particularly evident in the performance of \gls{sysc} on near-OOD datasets, which also require learning of ID class-relevant discriminative features. \gls{sysc}-MD improves AUC by 6.18\% over the nearest baseline (MD) and 7.29\% over \gls{sysa} in CIFAR-10. For TIN, \gls{sysc}-MD improves AUC by 8.28\% over the nearest baseline (CMG-e) and 5.29\% over \gls{sysa}. Variance of \gls{sysc} is decreased compared to \gls{sysa}, indicating that including learnable processing after view selection induces a stabilizing behavior. For example, the standard deviation of AUC for TIN in CIFAR-100 decreases from 4.61\% for \gls{sysa}-MD to 0.81\% for \gls{sysc}-MD. Notably, \gls{sysc} ID classification performance is comparable in the CIFAR-10 domain but decreases in the CIFAR-100 domain.
...
\gls{sysc} &  \textbf{89.97}$_{\pm 2.18}$ &  \underline{98.16}$_{\pm 0.46}$ & \underline{95.17}$_{\pm 1.54}$ & \underline{98.84}$_{\pm 0.64}$ & \underline{98.92}$_{\pm 0.58}$ & 96.48$_{\pm 0.81}$ &   \multicolumn{1}{|c|}{\textbf{96.26}}   \\ \hline
\end{tabular}
}
...
To investigate the source of performance improvements in \gls{sysc}, an ablation study is conducted over the various design components, presented in \ref{tab:propwork-scod-results-ablations-alt}. The alternating training paradigm improves over end-to-end training (End-to-End row), although the difference is primarily noticeable only in CIFAR-10 (where AUC increases by 9.76\%). It is hypothesized that the alternating training induces more stability in optimization over the loss surface, a factor that is conducive towards learning fine-grained discriminative features. The benefit is then evident in the near-OOD performance on CIFAR-10 while decreases in performance across other OOD datasets are comparatively diminished. Inclusion of the misalignment term also improves performance (No Misalignment row), with average OOD detection performance increasing by 0.66\%. Inducing diversity across $\tilde{z}_{fg}, \tilde{z}_{bg}$ ensures the presence of additional features for learning.

It is important to consider the possibility of using a shared CNN feature extractor backbone across all views, as it would provide significant parameter efficiency benefits (e.g. for edge computing). However, removing view-specific feature extractor backbones in \gls{sysc} significantly decreases performance, on average by 15.61\%. Addressing this issue is of interest towards the goal of real-world application-oriented OOD detection, and is considered in the next chapter (see Chapter \ref{diss_casod}). Overall, each component of \gls{sysc} plays a beneficial role.
...
This aside into exploring Attention-inspired architectural changes in FgBg multi-view OOD detection reveals the significant benefits of using mechanisms such as cross-attention. In particular, \gls{sysc}, which incorporates cross-attention as a feature fusion mechanism in the view selection pipeline, drastically increases OOD detection performance in the more-challenging CIFAR-100 experimental setting. The design improvements noted in \gls{sysa}, \gls{sysb}, and \gls{sysc} demonstrate that principled fusion of \emph{all} views - original, foreground, and background - via cross-attention is paramount for improving OOD detection with multiple segmented views. Following from the discussion in Sections \ref{sec:propwork-mod-results-takeaway} and \ref{sec:propwork-meo-results-takeaway} for \gls{sysa} and \gls{sysb}, it remains to fully expand the proposed ideas to large-scale datasets and modern Transformer-based architectures \cite{vaswani2017attention,dosovitskiy2020image}. Verifying the performance of these advanced, multi-view FgBg OOD detection models on challenging large-scale near-OOD settings would be a significant step towards the preparation of such solutions for real-world application. 
