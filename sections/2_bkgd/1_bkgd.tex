...
An \gls{id} training dataset $\mathcal{D}_{ID} = \{(x_i, y_i)\ |\ x_i \in \mathcal{x}_{ID},y_i \in \mathcal{Y}_{ID}\}_{i=1}^N$ is composed of samples $x_i \in \mathcal{X}_{ID}$ and labels $y_i \in \mathcal{Y}_{ID} = \{1,2,\dots,C\}$ for number of ID samples $N$ and number of ID classes $C$. $\mathcal{D}_{ID}$ is drawn \emph{i.i.d.} from an \gls{id} distribution $P_{ID}(X, Y)$, i.e. $\mathcal{D}_{ID} \sim P_{ID}$, which is the marginal distribution of joint data distribution $P_{\mathcal{X} \mathcal{Y}}$ on $\mathcal{X}_{ID}$. There also exists the \gls{ood} marginal distribution $P_{OOD}$ on \gls{ood} data $\mathcal{X}_{OOD}$ and unknown \gls{ood} labels $\mathcal{Y}_{OOD}$. An \gls{ood} dataset $\mathcal{D}_{OOD} = \{(x_i)\ |\ x_i \in \mathcal{X}_{OOD},y_i \in \mathcal{Y}_{OOD}\}^{L}_{i=1}$ is drawn from the \gls{ood} distribution $\mathcal{D}_{OOD} \sim P_{OOD}$ and contains samples $x_i \in \mathcal{X}_{OOD}$ with labels $y_i \in \mathcal{Y}_{OOD} = \{y | y \notin \mathcal{Y}_{ID}\}$ for number of OOD samples $L$.

The \gls{ood} detection task is, given \emph{only} $\mathcal{D}_{ID}$ and a test sample $x'$, identify if the test label $y' \in \mathcal{Y}_{ID}$ or $y' \in \mathcal{Y}_{OOD}$. An \gls{ood} detection model $G$ uses an ML model $f$ in the following formulated level set estimation:
\begin{equation}
  G(x', f) =
  \begin{cases}
    1 \text{\ (OOD),} & \text{$S(x'; f) \geq \lambda$} \\
  ...
  \end{cases}
\end{equation}
for scoring function $S(\cdot)$ and user-defined threshold $\lambda$. Of note is that the \gls{ood} score $S$ is \textit{higher} for samples identified as \textit{more \gls{id}} and $S$ is \textit{lower} for samples identified as \textit{more \gls{ood}}.
...
Several metrics have been proposed to measure the performance of \gls{ood} detection methods \cite{lu2024recent}:
\begin{itemize}
    \item \textbf{Area Under the Receiver Operating Characteristic Curve (AUC).} This metric quantifies the model likelihood of assigning higher scores to \gls{id} samples with respect to scores of \gls{ood} samples. AUC correlates with the model's ability to correctly rank \gls{id} samples higher than \gls{ood} samples. A \textbf{higher} AUC is indicative of better \gls{ood} detection performance.
    \item \textbf{False Positive Rate at 95\% True Positive Rate (FPR95).} This metric quantifies the false positive rate (FPR) at the value where the true positive rate (TPR) becomes 95\%. It measures the amount of \gls{ood} samples incorrectly classified as \gls{id}. FPR95 correlates with an \gls{ood} detection model's likelihood of false detections under strict \gls{id} detection requirements. A \textbf{lower} FPR95 is indicative of better \gls{ood} detection performance.
    \item \textbf{Area Under the Precision-Recall Curve (AUPR).} This metric quantifies a combination of model precision and recall. Two versions of AUPR exist: one where precision and recall is considered for \gls{id} data as the ``correct'' class of interest (AUPR-IN) and one where \gls{ood} data is considered as the ``correct'' class of interest (AUPR-OUT). AUPR is particularly effective at measuring \gls{ood} detection performance under severe \gls{id}-\gls{ood} data imbalance. AUPR correlates directly with the model's ability to correctly classify \gls{id} (or \gls{ood}) data. A \textbf{higher} AUPR is indicative of better \gls{ood} detection performance.
    %\item \textbf{Detection Error (DE).} This metric quantifies a combination of model precision and recall. Two versions of AUPR exist: one where precision and recall is considered for \gls{id} data as the ``correct'' class of interest (AUPR-IN) and one where \gls{ood} data is considered as the ``correct'' class of interest (AUPR-OUT). In particular, AUPR effectively measures \gls{ood} detection performance under severe \gls{id}-\gls{ood} data imbalance. AUPR correlates directly with the model's ability to correctly classify \gls{id} (or \gls{ood}) data. A \textbf{higher} AUPR is indicative of better \gls{ood} detection performance.
\end{itemize}

Of note is that the \gls{ood} detection literature more frequently makes use of the AUC score \cite{bradley1997use,hendrycks2017baseline,lee2018simple}. AUC covers general model performance and avoids reliance on a user-defined threshold which may be application- or domain-dependent \cite{hendrycks2017baseline}. This thesis likewise prioritizes AUC as the predominant OOD detection metric but other metrics are reported in varying capacities as needed to illustrate robustness.

\section{Relevant OOD Detection Methods}

\subsection{Maximum Softmax Probability}

The paper proposing Maximum Softmax Probability (MSP) \cite{hendrycks2017baseline} is widely considered to be the work spawning the \gls{ood} Detection field. MSP uses a classification model $f$ and treats the predicted probability distribution, $\text{softmax}(\hat{y})$, as a representation of model confidence in the sample $x$ under consideration, where ``$\text{softmax}$'' is the softmax function and $\hat{y} = f(x) \in \mathbb{R}^C$ is the predicted logits. The maximum probability is used as the \gls{ood} score
\begin{equation}
\begin{aligned}
    S_{MSP}(x,f) = \max_c\ \text{softmax}(f(x)), \\
\end{aligned}
\end{equation}
with $\max\limits_{c}$ indicating the maximum function over probabilities of classes $c \in C$. MSP stands as a simple \gls{ood} detection tool that remains as a reliable baseline, although much research has revealed limitations to the method \cite{hendrycks2019scaling} and proposed improvements \cite{sun2021react}.
...
%\end{aligned}
%\end{equation}
Mahalanobis Distance (MD) \cite{lee2018simple} is considered the original distance-based \gls{ood} detection method, measuring the distance between a sample $x$ and Gaussian distributions representing \gls{id} data in the feature space. MD first computes class-conditional Gaussian distributions for each \gls{id} class in the training distribution. This is intractable as it cannot be assumed that any model has access to the entire \gls{id} training data distribution. The authors of MD propose instead to approximately fit the \gls{id} class Gaussian distributions using maximum likelihood estimation over the available training data and tied covariance.
...
Given the class means $\hat{\mu}_{C}$ and covariance matrix $\hat{\Sigma}$, the MD \gls{ood} score is calculated using the feature vector $z' = \phi(x')$ for test sample $x'$
\begin{equation}
\begin{aligned}
     S_{MD}(x',\phi) = \max_{c} (-(z' - \hat{\mu}_{c})^\top \hat{\Sigma}^{-1} (z' - \hat{\mu}_{c})). \\
\end{aligned}
\end{equation}
$\max\limits_{c}$ again indicates the maximum function over classes $c \in C$. MD continues to demonstrate impressive performance in many \gls{ood} detection domains \cite{yang2022openood,zhang2023openood,ren2021simple}. Importantly, as MD is a distance-based \gls{ood} detection method that relies on features (also referred to as a ``feature-based OOD detection method'' in this thesis), its performance on \gls{ood} detection improves as the embedded feature space (henceforth referred to simply as ``feature space'') improves. That is, as the model learns increasingly discriminative, \gls{id} class-relevant features, \gls{id} sample features cluster into localized groups (according to class) with increasingly distinct separation in the feature space. MD accurately models this feature space and thus separates \gls{ood} sample features from \gls{id} sample features more clearly \cite{lee2018simple,sun2022out,ammar2024neco,fort2021exploring}. Orthogonal techniques that improve learning of features from data, e.g. data augmentation, also benefit MD (and other feature-based \gls{ood} detection methods) \cite{sun2022out,khosla2020supervised,hendrycks2022pixmix,yun2019cutmix}.
...
k-Nearest Neighbors (KNN) \cite{sun2022out} is another distance-based \gls{ood} detection method. However, it differs from MD in that it uses a non-parameteric distance metric. Specifically, KNN uses the $k$-th nearest neighbor distance between a sample $x$ and the training dataset feature vectors. An illustration of the similarities and differences between MD and KNN is in \ref{fig:bkgd-mdknn}.

\gls{id} training feature vectors are collected and normalized (per findings from a related investigation \cite{tack2020csi,huang2021importance}) from the training dataset $\mathcal{D}_{ID}$, $\mathcal{Z} = \{\zeta_{i}\}^{N}$ with $\zeta = \frac{z}{\lvert \rvert z \lvert \rvert_{2}}$ as the $L_2$ norm of feature vector $z$. The KNN \gls{ood} score is calculated using the normalized feature vector $\zeta'$ for test sample $x'$. Specifically, the Euclidean distances between $\zeta'$ and each $\zeta_i \in \mathcal{Z}$, $d_{KNN,i}(\zeta', \zeta_i) = \lvert \rvert \zeta_{i} - \zeta' \lvert \rvert_{2}$, is computed and ranked according to distance, $D_{KNN}' = \{d_{KNN,(i)}\}^{N}$ where $(i)$ represents the newly ordered position. The $k$-th Euclidean distance is used as the \gls{ood} score
\begin{equation}
\begin{aligned}
    S_{KNN}(x',\mathcal{Z},k) = -d_{KNN,(k)}(\zeta', \zeta_{(k)}). \\
\end{aligned}
\end{equation}
Note that KNN does not require the labels of the \gls{id} data, i.e. it is unsupervised. This has certain advantages in settings without guaranteed access to labeled data. As with MD, KNN improves as the feature space improves. Furthermore, KNN makes no assumptions about the distributions of data in the feature space.
...
Virtual Logit Matching (VIM) \cite{wang2022vim} is a hybrid \gls{ood} detection approach that uses both features and predicted logits to quantify an \gls{ood} score. VIM divides the feature vector $z$ of a sample $x$ into a column subspace (CS) component and a null subspace (NS) component before comparing the NS component to predicted logits $\hat{y}$.

The detailed procedure is as follows: given feature extractor $\phi$ of model $f$ and feature vector $z = \phi(x)$ for input sample $x$, the model can be further specified as $f(x) = W \phi(x) + b$ for classification linear layer weight matrix $W \in \mathbb{R}^{d \times C}$ and bias $b \in \mathbb{R}^{C}$, where $d$ is the dimension of feature vector $z \in \mathbb{R}^d$. $W$ can be decomposed into a CS $W_{cs}$ and an NS $W^{\perp}$. $W^{\perp}$ is unique in that it does not affect classification but it influences \gls{ood} detection due to the definition of NS. Specifically, the NS $W^{\perp} = \text{Nul}\ W$ is the set of solutions satisfying $W z = 0$. $z$ can be similarly decomposed into $z= z^{W^{\perp}} + z^{W_{cs}}$ with $z^{W^{\perp}}$ and $z^{W_{cs}}$ as projections of $z$ to the NS $W^{\perp}$ and CS $W_{cs}$ respectively.

NS projections $z^{W^{\perp}}$ in \gls{ood} samples deviate significantly with those in \gls{id} samples. To exploit this, VIM directly compares the NS projection with predicted logits. First, the NS projection is redefined as a virtual logit $\hat{y}_0 = \alpha \| z^{W^{\perp}} \|$, with $\alpha$ as a scaling parameter computed from virtual logits and maximum predicted logits of the \gls{id} data
\begin{equation}
\begin{aligned}
    \alpha = \frac{\sum_{i=1}^{N} \max_{j=1,\dots,C} \hat{y}_j^i}{\sum_{i=1}^{N} \| z^{W^{\perp}} \|}. \\
\end{aligned}
\end{equation}
Then, the virtual logit is concatenated to the predicted logits and a softmax is performed to retrieve the NS projection sample probability. The probability of the virtual logit is used as the \gls{ood} score
\begin{equation}
\begin{aligned}
    S_{VIM}(z,\hat{y}) = \frac{e^{\hat{y}_0}}{\sum_{i=1}^{N} e^{\hat{y}_i} + e^{\hat{y}_0}}. \\
\end{aligned}
\end{equation}
VIM has consistently shown state-of-the-art performance despite many later works proposing alternative approaches for \gls{ood} detection. The combination of feature and logits information increases the saliency of information for OOD detection in any given evaluated sample.
